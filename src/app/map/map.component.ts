import { Component, OnInit, ViewChild, ElementRef, OnDestroy, Output, TrackByFunction } from '@angular/core';
...
export class MapComponent implements OnInit, OnDestroy {
  ...
  private qualifyingNeighbourhood = 0;
  private currentTime = 15;
  private currentTimeString = '15mins';
  ...
  ngOnInit() {
    ...
    this.map.on('load', (event) =>  {
      ...
      .subscribe(updatedData => {     
        ...
        /** First get a bounding box to get extreme X and Y points 
         * that are required to build the Square Grid */
        let line = turf.lineString(updatedData.isoResponse.features[0].geometry.coordinates[0]);
...
clearMarkers() {
  ...
  let layers = [  'hospital_layer', 'school_layer', 'park_layer', 'supermarket_layer', 'train_station_layer', 'tourist_attraction_layer', 'bus_stop_layer' ];
  let sources = [  'hospital_source', 'school_source', 'park_source', 'supermarket_source', 'train_station_source', 'tourist_attraction_source', 'bus_stop_source'];
